...
export default function VolunteerTasksPage() {
    ...
    const activeTasks = tasks.filter((t: any) => ['assigned', 'accepted', 'picked'].includes(t.status));
    const pastTasks = tasks.filter((t: any) => ['delivered', 'declined'].includes(t.status));
                                        ...
                                        <Card key={task._id} className={`overflow-hidden transition-all ${isActive ? 'ring-1 ring-emerald-500 shadow-md' : ''}`}>
                                            <CardContent className="p-0">
                                                <div className="p-4">
                                                        ...
                                                        <div className="flex flex-wrap gap-2 pt-2">
                                                            {task.status === 'assigned' && (
                                                                <Button
                                                                    ...
                                                                    disabled={actionLoading === task._id || (donation?.expiryTime && new Date(donation.expiryTime).getTime() < Date.now())}
                                                                >
                                                                    {actionLoading === task._id ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Check className="w-4 h-4 mr-2" />}
                                                                    {(donation?.expiryTime && new Date(donation.expiryTime).getTime() < Date.now()) ? 'Cannot Accept (Expired)' : 'Accept Task'}
                                                                </Button>
                                                            )}
                                                    ...
                                                    {isActive && (
                                                        <div className="flex gap-1 mt-4 border-t pt-4">
                                                            <Button
                                                                variant="ghost"
                                                                size="sm"
                                                                onClick={() => setShowHistoryForTask(showHistoryForTask === task._id ? null : task._id)}
                                                                className={`h-9 gap-2 ${showHistoryForTask === task._id ? 'text-emerald-600 bg-emerald-50' : 'text-gray-400'}`}
                                                            >
                                                            ...
                                                            <Button
                                                                ...
                                                                className={`h-9 gap-2 ${showMapForTask === task._id ? 'text-emerald-600 bg-emerald-50' : 'text-gray-400'}`}
                                                            >
                                                                {showMapForTask === task._id ? <X className="w-4 h-4" /> : <MapIcon className="w-4 h-4" />} Map
                                                            </Button>
                                                        </div>
                                                    )}
                                                </div>
